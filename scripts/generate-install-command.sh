...
# do node obtido da API (servidor). Uso: ./scripts/generate-install-command.sh [NODE_UID]
#
...
if [[ -f "$SHA256_FILE" ]]; then
  ...
  echo "Aviso: $SHA256_FILE não encontrado; use SHA256 do artefato publicado." >&2
  SHA256_VALUE=""
fi

# Base URL do repo (raw GitHub)
REPO_RAW_BASE="${GITHUB_RAW_BASE:-https://raw.githubusercontent.com/pablomichelin/pfsense-monitor-agent/main}"
