...
smoke_client_sql="(
  code ~ '^(SCOPE-[AB]|CPROF-[AB]|RBAC|RBAC-ND|PERM|BK|UX-CLIENT|AUDIT|DBG|DL|REQ|RET)-[0-9]+\$'
  OR name ~ '^(Scope [AB]|Client Profile [AB]|RBAC Smoke|RBAC Node Detail|Perm Smoke|Backup Smoke|UX Client|Audit |Dbg )[0-9]+\$'
  OR name = 'DBG'
)"

smoke_user_sql="(
  email ~ '@test\\.local\$'
  OR email ~ '^(scoped-admin|perm-readonly|perm-operator|readonly|operator|client-user|audit-ro|ux-client|dbg-scoped)-[0-9]+@'
)"
