...
function mapSkipReason(reason: string | null): string {
  ...
  switch (reason) {
    ...
    case 'backup queued before provision':
      return 'Gerando backup do config.xml — o usuário será aplicado em seguida';
...
function resolveBackupQueuedRow(pipeline?: BackupPipelineState): {
  outcomeLabel: string;
  statusLabel: string;
  detail: string;
} {
  if (!pipeline) {
    return {
      outcomeLabel: 'Backup em andamento',
      statusLabel: 'Aguardando backup',
      detail: 'Gerando backup do config.xml — o usuário será aplicado em seguida',
    };
  }

  const backupInFlight =
    !pipeline.backupStatus ||
    pipeline.backupStatus === 'pending' ||
    pipeline.backupStatus === 'picked_up' ||
    pipeline.backupStatus === 'running';
  ...
  if (
    ...
    const fallback =
      pipeline.backupStatus === 'expired'
        ? 'command expired before backup follow-up'
        : 'Falha no backup — o provisionamento não foi iniciado';
    return {
      outcomeLabel: 'Falha no backup',
      statusLabel: mapCommandStatusLabel(pipeline.backupStatus),
      detail: mapCommandDetail(pipeline.backupError, fallback),
    };
  }
  ...
  if (backupInFlight) {
    return {
      outcomeLabel: 'Backup em andamento',
      statusLabel: `${mapCommandStatusLabel(pipeline.backupStatus || 'pending')} (backup)`,
      detail: 'Gerando backup do config.xml — o usuário será aplicado em seguida',
    };
  }
  ...
  if (pipeline.accountStatus === 'active') {
    return {
      outcomeLabel: 'Provisionado',
      statusLabel: mapAccountStatusLabel(pipeline.accountStatus),
      detail: 'Usuário aplicado no firewall',
    };
  }
  ...
  if (pipeline.accountStatus === 'failed') {
    return {
      outcomeLabel: 'Falha',
      statusLabel: mapAccountStatusLabel(pipeline.accountStatus),
      detail: mapCommandDetail(
        pipeline.accountError,
        'Falha ao aplicar o usuário após o backup',
      ),
    };
  }

  if (
    pipeline.accountStatus === 'pending_create' ||
    pipeline.accountStatus === 'password_reset_pending' ||
    pipeline.backupStatus === 'succeeded'
  ) {
    return {
      outcomeLabel: 'Backup concluído',
      statusLabel: pipeline.accountStatus
        ? mapAccountStatusLabel(pipeline.accountStatus)
        : 'Provisionando',
      detail: 'Backup concluído — aplicando usuário no firewall',
    };
  }

  return {
    outcomeLabel: 'Backup em andamento',
    statusLabel: `${mapCommandStatusLabel(pipeline.backupStatus)} (backup)`,
    detail: 'Gerando backup do config.xml — o usuário será aplicado em seguida',
  };
}
...
export function FleetTechnicianManagementPanel({
  ...
  const [backupPipelineByNodeId, setBackupPipelineByNodeId] = useState<
    Map<string, BackupPipelineState>
  >(new Map());
    ...
    try {
      const entries = await Promise.all(
        backupQueuedItems.map(async (item) => {
          ...
          let accountError: string | null | undefined;
          ...
          if (backupStatus.status === 'succeeded') {
            ...
            accountError = account?.last_error;
          }
          ...
          return [
            item.node_id,
            {
              ...
              backupError: backupStatus.error_message,
              accountStatus,
              accountError,
            },
          ] as const;
      ...
      const next = new Map<string, BackupPipelineState>();
  ...
  const resultRows = useMemo(() => {
    ...
    return initialResponse.results.map((item) => {
      const live = liveStatusByNodeId.get(item.node_id);
      if (item.outcome === 'backup_queued') {
        return {
          item,
          ...resolveBackupQueuedRow(backupPipelineByNodeId.get(item.node_id)),
        };
      }

      return {
        item,
        outcomeLabel: mapOutcomeLabel(item.outcome),
        statusLabel: live ? mapCommandStatusLabel(live.status) : '—',
        detail: mapCommandDetail(live?.error_message, item.reason),
      };
    });
  }, [initialResponse, liveStatusByNodeId, backupPipelineByNodeId]);

  const liveBackupSummary = useMemo(() => {
    const backupRows = resultRows.filter((row) => row.item.outcome === 'backup_queued');
    if (backupRows.length === 0) {
      return null;
    }

    const provisioned = backupRows.filter((row) => row.outcomeLabel === 'Provisionado').length;
    const failed = backupRows.filter(
      (row) => row.outcomeLabel === 'Falha' || row.outcomeLabel === 'Falha no backup',
    ).length;
    const pending = backupRows.length - provisioned - failed;

    if (failed > 0 && pending === 0) {
      return {
        variant: 'error' as const,
        text: `${provisioned} provisionado(s) após backup, ${failed} falha(s) no pipeline de backup`,
      };
    }
    if (pending === 0) {
      return {
        variant: 'success' as const,
        text: `${provisioned} provisionado(s) após backup`,
      };
    }
    return {
      variant: 'info' as const,
      text: `${pending} backup(s) em andamento — o status atualiza ao concluir`,
    };
  }, [resultRows]);
  ...
  const batchFinished =
    ...
    (backupQueuedItems.length === 0 ||
      backupQueuedItems.every((item) => {
        ...
        if (pipeline.backupStatus !== 'succeeded') {
          return true;
        }
        return (
          pipeline.accountStatus === 'active' ||
          pipeline.accountStatus === 'failed'
        );
      }));
    ...
    <Card className="space-y-4 p-4">
      ...
      {!initialResponse ? (
        ...
        <div className="space-y-3">
          ...
          <Alert
            variant={
              liveBackupSummary?.variant ??
              (initialResponse.summary.failed > 0
                ? 'error'
                : initialResponse.summary.enqueued > 0
                  ? 'success'
                  : 'warning')
            }
          >
          ...
          {batchStatuses.length > 0 || backupQueuedItems.length > 0 ? (
            <p className="text-sm text-slate-300">
              Progresso:{' '}
              {batchStatuses.length > 0 ? (
                <>
                ...
                <>{liveBackupSummary?.text ?? 'acompanhando backup e provisionamento'}</>
              )}{' '}
              —{' '}
          ...
          <div className="overflow-x-auto rounded-lg border border-slate-800">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {resultRows.map(({ item, outcomeLabel, statusLabel, detail }) => {
