  ...
  const runBatchAction = () => {
    ...
    startTransition(async () => {
      try {
        ...
        const autoGenerated = !passwordValue;
        let response: BatchResponse;

        if (actionMode === 'provision') {
          setLastActionLabel('Provisionar');
          response = await createTechnicianBatchProvisionAction({
          ...
          response = await createTechnicianBatchPasswordResetAction({
            technician_id: technicianId,
            node_ids: nodeIds,
            password: passwordValue,
            client_id: clientId,
    ...
    <Card className="space-y-4 p-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Cadastro centralizado e ações em lote nos firewalls selecionados. Requer agente{' '}
          {MIN_AGENT_VERSION}+ com gestão de técnicos habilitada no pfSense.
        </p>
      </div>
        ...
        <>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      startDeleteTransition(async () => {
                        try {
                          await deleteTechnicianFromRegistryAction({
                          ...
                          setError(
                            err instanceof Error
                              ? err.message
                              : 'Falha ao remover técnico do cadastro',
                          );
                        }
                      });
                    }}
          ...
          {technicians.length > 0 && nodeIds.length > 0 ? (
                ...
                {actionMode !== 'revoke' ? (
                  <label className="flex flex-1 flex-col gap-1 text-sm text-slate-300">
                    ...
                    <input
                      ...
                      placeholder="Mín. 12 caracteres ou deixe vazio"
                      className="h-11 rounded-lg border border-slate-600/80 bg-panel-soft px-3 text-sm text-slate-200"
            ...
            <Alert variant="warning">
              Selecione firewalls na tabela acima ou ajuste o filtro para executar ações em lote.
            </Alert>
          ) : null}
        </>
      ) : (
